      ...
      case "library":
        return (
          ...
          >
            ...
            {/* ADDED: Vertical data streams for library organization */}
            <div className="absolute inset-0" style={{ zIndex: 0 }}>
              {[...Array(10)].map((_, i) => (
                <div
                  key={`library-stream-${i}`}
                  className={`absolute w-px ${isDarkMode ? "bg-gradient-to-b from-purple-400/50 via-purple-400/25 to-transparent" : "bg-gradient-to-b from-purple-600/30 via-purple-600/15 to-transparent"}`}
                  style={{
                    height: `${50 + (i % 3) * 25}px`,
                    left: `${10 + i * 8}%`,
                    top: `${5 + (i % 4) * 20}%`,
                    transform: `translateY(${mousePosition.y * (12 + i * 1.5)}px)`,
                    animation: `libraryStreamDown ${2 + (i % 4) * 0.5}s linear infinite`,
            ...
            {/* ADDED: Diagonal organization lines */}
            <div className="absolute inset-0" style={{ zIndex: 0 }}>
              {[...Array(6)].map((_, i) => (
                <div
                  key={`library-diagonal-${i}`}
                  className={`absolute w-px ${isDarkMode ? "bg-gradient-to-br from-purple-400/40 via-purple-400/20 to-transparent" : "bg-gradient-to-br from-purple-600/25 via-purple-600/12 to-transparent"}`}
                  style={{
                    height: `${80 + (i % 2) * 30}px`,
                    left: `${15 + i * 12}%`,
                    top: `${10 + (i % 3) * 25}%`,
                    transform: `translate(${mousePosition.x * (8 + i * 2)}px, ${mousePosition.y * (10 + i)}px) rotate(${25 + i * 10}deg)`,
                    animation: `librarySlide ${3 + (i % 3) * 0.7}s ease-in-out infinite alternate`,
            ...
            {/* ADDED: Connection network lines */}
            <div className="absolute inset-0" style={{ zIndex: 0 }}>
              {[...Array(8)].map((_, i) => (
                <div
                  key={`library-connect-${i}`}
                  className={`absolute h-px ${isDarkMode ? "bg-gradient-to-r from-transparent via-purple-400/30 to-transparent" : "bg-gradient-to-r from-transparent via-purple-600/20 to-transparent"}`}
                  style={{
                    width: `${60 + (i % 4) * 20}px`,
                    left: `${8 + (i % 5) * 18}%`,
                    top: `${25 + i * 6}%`,
                    transform: `translateX(${mousePosition.x * (6 + i * 2)}px)`,
                    animation: `libraryConnect ${2.5 + (i % 3) * 0.6}s linear infinite`,
      ...
      case "realtime":
        return (
          ...
          >
            ...
            {/* Animated streaming data lines */}
            <div className="absolute inset-0" style={{ zIndex: 0 }}>
              {[...Array(12)].map((_, i) => (
                <div
                  key={i}
                  className={`absolute w-px ${isDarkMode ? "bg-gradient-to-b from-emerald-400/60 via-emerald-400/30 to-transparent" : "bg-gradient-to-b from-emerald-600/40 via-emerald-600/20 to-transparent"}`}
                  style={{
                    height: `${60 + (i % 4) * 20}px`,
                    left: `${15 + i * 6}%`,
                    top: `${10 + (i % 3) * 15}%`,
                    transform: `translateY(${mousePosition.y * (15 + i * 2)}px)`,
                    animation: `streamDown ${1.5 + (i % 5) * 0.4}s linear infinite`,
            ...
            <div className="absolute inset-0" style={{ zIndex: 0 }}>
              {[...Array(8)].map((_, i) => (
                <div
                  key={`horizontal-${i}`}
                  className={`absolute h-px ${isDarkMode ? "bg-gradient-to-r from-transparent via-emerald-400/40 to-transparent" : "bg-gradient-to-r from-transparent via-emerald-600/30 to-transparent"}`}
                  style={{
                    width: `${80 + (i % 3) * 30}px`,
                    left: `${5 + (i % 4) * 20}%`,
                    top: `${20 + i * 8}%`,
                    transform: `translateX(${mousePosition.x * (10 + i * 3)}px)`,
                    animation: `streamRight ${2 + (i % 4) * 0.5}s linear infinite`,
                    animationDelay: `${i * 0.3}s`,
                    zIndex: 0,
                  }}
                />
    ...
    >
      ...
      >
        {currentFeature.isFooter ? (
          ...
          <div
            className="text-center max-w-4xl mx-auto transform transition-all duration-700 ease-out"
            style={{
          ...
          >
            ...
            <div className="flex justify-center mb-6 sm:mb-12">
              <div className="relative group">
                <div
                  className={`w-20 h-20 sm:w-32 sm:h-32 ${currentFeature.accentBg} backdrop-blur-sm rounded-2xl sm:rounded-3xl flex items-center justify-center border ${isDarkMode ? "border-white/10" : "border-black/10"} transition-all duration-500 hover:scale-105`}
                >
                  <IconComponent
                    className={`w-10 h-10 sm:w-16 sm:h-16 ${currentFeature.accent} transition-all duration-500`}
                  />
                </div>
                <div
                  className={`absolute inset-0 w-20 h-20 sm:w-32 sm:h-32 ${currentFeature.accentBg} rounded-2xl sm:rounded-3xl blur-xl opacity-50 group-hover:opacity-70 transition-opacity duration-500`}
                />
              </div>
            </div>
            ...
            {/* Title */}
            <h1
              className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-light mb-4 sm:mb-6 md:mb-8 ${isDarkMode ? "text-white" : "text-gray-900"} transition-all duration-700 leading-tight`}
            >
            ...
            {/* Description */}
            <p
              className={`text-base sm:text-lg md:text-xl lg:text-2xl mb-8 sm:mb-12 md:mb-16 max-w-xs sm:max-w-lg md:max-w-2xl mx-auto leading-relaxed ${isDarkMode ? "text-gray-300" : "text-gray-600"} transition-all duration-700 px-4 sm:px-0`}
            >
            ...
            <div className="flex flex-col items-center space-y-3 sm:space-y-4">
              ...
              >
                ...
                />
                <div className="relative flex items-center px-6 sm:px-8 md:px-12 py-3 sm:py-4">
                  <span
                    className={`text-sm sm:text-base md:text-lg font-medium ${currentFeature.accent} group-hover:translate-x-2 transition-transform duration-300`}
                  >
                    Get Started
                  </span>
                  <svg
                    className={`w-4 h-4 sm:w-5 sm:h-5 ml-3 sm:ml-4 ${currentFeature.accent} group-hover:translate-x-2 transition-transform duration-300`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              {currentFeature.requiresAuth && !user && (
                <div className="text-center opacity-60 transition-opacity duration-300">
                  <span
                    className={`text-xs sm:text-sm ${isDarkMode ? "text-white/70" : "text-gray-500"} font-light`}
                  >
                    • Login required
                  </span>
                </div>
      ...
      {/* Navigation dots */}
      <div
        className="absolute bottom-4 sm:bottom-6 md:bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2 sm:space-x-3 md:space-x-4"
        style={{ zIndex: 3 }}
      >
